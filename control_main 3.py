...
# -----------------------------------------------------------------------------
# Global exception handler (WebUI expects JSON, not HTML/plain-text).
# IMPORTANT: must be declared after `app = FastAPI(...)` to avoid NameError.
# -----------------------------------------------------------------------------
@app.exception_handler(Exception)
async def _control_unhandled_exception_handler(request: Request, exc: Exception):
    # Let FastAPI's built-in HTTPException/validation handlers run first (they are more specific).
    logger.exception("CONTROL_UNHANDLED_EXCEPTION method=%s path=%s", request.method, request.url.path)
    rid = request.headers.get("x-request-id") or request.headers.get("x-vozlia-request-id")
